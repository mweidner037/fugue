...
import { FugueTextMessage, FugueTextSave } from "../generated/proto_compiled";
import {
  ListPosition,
  ListPositionSource,
...
export interface FugueTextInsertEvent extends CollabEvent {
  startIndex: number;
  count: number;
}

export interface FugueTextDeleteEvent extends CollabEvent {
  startIndex: number;
  count: number;
  deletedValues: string;
}

export interface FugueTextEventsRecord extends CollabEventsRecord {
  Insert: FugueTextInsertEvent;
  Delete: FugueTextDeleteEvent;
}

export class FugueText extends CPrimitive<FugueTextEventsRecord> {
   ...
   * local echos, i.e., the FugueText is used in a non-CRDT fashion.
   */
  private indexHint = -1;
  ...
  insert(index: number, values: string): void {
    ...
    const message = FugueTextMessage.create({
      insert: {
        counter,
        startValueIndex,
        metadata,
    ...
    this.sendPrimitive(FugueTextMessage.encode(message).finish());
  }
  ...
  protected receivePrimitive(
    ...
    const decoded = FugueTextMessage.decode(<Uint8Array>message);
    switch (decoded.op) {
      case "insert": {
  ...
  *values(): IterableIterator<string> {
  ...
  savePrimitive(): Uint8Array {
    const message = FugueTextSave.create({
      positionSourceSave: this.positionSource.save(),
      valuesSave: this.toString(),
    });
    return FugueTextSave.encode(message).finish();
  }

  loadPrimitive(saveData: Uint8Array | null): void {
    if (saveData === null) return;

    const decoded = FugueTextSave.decode(saveData);
