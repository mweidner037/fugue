 ...
 *
 * `Fugue<T>` has a similar API to `Array<T>`,
 ...
 * instead use a [[CList]].
 *
 * It is *not* safe to modify a Fugue while iterating over it. The iterator
 ...
 */
export class Fugue<T> extends AbstractList_CObject<T, [T]> {
  /**
  ...
  private altReceivePrimitive(message: Uint8Array, meta: MessageMeta): void {
    const decoded = FugueMessage.decode(message);
    switch (decoded.op) {
      case "insert": {
        const insert = nonNull(decoded.insert) as FugueInsertMessage;
        const values =
          insert.data === "value"
  ...
  insert(index: number, ...values: T[]): T | undefined {
    ...
    const insertMessage: IFugueInsertMessage = {
    ...
    this.messenger.sendMessage(
      FugueMessage.encode({ insert: insertMessage }).finish()
    );
  ...
  delete(index: number, count = 1): void {
    ...
    for (let i = index + count - 1; i >= index; i--) {
      this.messenger.sendMessage(
        FugueMessage.encode({ delete: this.list.getPosition(i) }).finish()
      );
    }
  }
  ...
  load(savedStateTree: SavedStateTree | null, meta: SavedStateMeta): void {
    ...
    if (this.list.inInitialState) {
      ...
      // present in it.
      // Those must have been deleted by (a replica of) us (= this Fugue).
