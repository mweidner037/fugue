import {
  ...
  DefaultSerializer, InitToken, int64AsNumber, Message,
  MessageMeta, Optional, PositionedList, Serializer
} from "@collabs/core";
import {
  ITreeFugueArrayInsertMessage,
  ITreeFugueArraySave,
  TreeFugueArrayMessage,
  TreeFugueArraySave
  ...
  implements PositionedList
{
  ...
  insert(index: number, ...values: T[]): T | undefined {
    ...
    const imessage: ITreeFugueArrayInsertMessage = {
      counter,
      startValueIndex,
      metadata,
    };
    ...
    const message = TreeFugueArrayMessage.create({
      insert: imessage,
    });
    this.sendPrimitive(TreeFugueArrayMessage.encode(message).finish());
  ...
  protected receivePrimitive(message: Message, meta: MessageMeta): void {
    ...
    const decoded = TreeFugueArrayMessage.decode(<Uint8Array>message);
    switch (decoded.op) {
      case "insert": {
  ...
  *positionEntries(): IterableIterator<[string, T]> {
  ...
  save(): Uint8Array {
    const imessage: ITreeFugueArraySave = {
    ...
    const message = TreeFugueArraySave.create(imessage);
    return TreeFugueArraySave.encode(message).finish();
  }

  load(saveData: Optional<Uint8Array>): void {
    if (saveData.isPresent) {
      const decoded = TreeFugueArraySave.decode(saveData.get());
      let values: T[];
      if (this.valueArraySerializer !== undefined) {
