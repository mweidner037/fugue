...
export const runBenchmarkB4 = async (crdtFactory, filter) => {
  async function benchmarkTemplateTrial(
    id,
    inputData,
    changeFunction,
    check,
    ...
    await (async () => {
      ...
      // Experimentally, sleeping before getMemUsed() makes memory msmts more reliable -
      // otherwise may get negative diffs (last trial getting GC'd in the middle?).
      await new Promise((resolve) => setTimeout(resolve, 1000));
      const startMemUsed = getMemUsed();

      let doc1 = crdtFactory.create((update) => {
  ...
  const benchmarkTemplate = async (id, inputData, changeFunction, check) => {
    for (let trial = -WARMUP_TRIALS; trial < MEASURED_TRIALS; trial++) {
      // Call a separate function to keep old trials' states out of scope.
      await benchmarkTemplateTrial(id, inputData, changeFunction, check, trial);
    }
  };

  await runBenchmark(
    "[B4] Apply real-world editing dataset",
    filter,
    async (benchmarkName) => {
  ...
  await runBenchmark(
    "[B4.2] Measure docSize over time for real-world editing dataset",
    filter,
    async (benchmarkName) => {
      const textSizes = new Array(
        Math.floor(edits.length / OVER_TIME_INTERVAL)
      );
      ...
      for (let trial = -WARMUP_TRIALS; trial < MEASURED_TRIALS; trial++) {
        ...
        if (trial === -WARMUP_TRIALS) {
          setBenchmarkResult(
            crdtFactory.getName(),
            `${benchmarkName} (textSizeOverTime)`,
            textSizes.join(",")
          );
        }
  ...
  async function b43Trial(benchmarkName, memUseds, trial) {
    // Experimentally, sleeping before getMemUsed() makes memory msmts more reliable -
    // otherwise may get negative diffs (last trial getting GC'd in the middle?).
    await new Promise((resolve) => setTimeout(resolve, 1000));
    const startMemUsed = getMemUsed();
  ...
  await runBenchmark(
    "[B4.3] Measure memUsed over time for real-world editing dataset",
    filter,
    async (benchmarkName) => {
      const memUseds = new Array(Math.floor(edits.length / OVER_TIME_INTERVAL));
      for (let trial = -WARMUP_TRIALS; trial < MEASURED_TRIALS; trial++) {
        // Call a separate function to keep old trials' states out of scope.
        await b43Trial(benchmarkName, memUseds, trial);
      }
    }
  );

  async function b4x100Trial(benchmarkName, multiplicator, trial) {
    ...
    await (async () => {
      ...
      let doc = crdtFactory.create((update) => {
        updateSize += update.length;
      });
  ...
  await runBenchmark(
    "[B4x100] Apply real-world editing dataset 100 times",
    filter,
    async (benchmarkName) => {
      const multiplicator = 100;
      for (let trial = -WARMUP_TRIALS; trial < MEASURED_TRIALS; trial++) {
        // Call a separate function to keep old trials' states out of scope.
        await b4x100Trial(benchmarkName, multiplicator, trial);
      }
    }
  );
};
